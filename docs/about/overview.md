...
| **Group Chat**            | Multi-agent conversation where you delegate the moderating to an agent. It routes to the right agents, follows up when an answer is thin, and runs as many rounds as the question needs before synthesizing.                                                     |
| **Cross-Agent Mention**   | `@another-agent` typed in an ordinary chat. A single-turn consult: the mentioned agent answers once and stops, and you stay the moderator for anything that follows.                                                                                             |
